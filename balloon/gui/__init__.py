...
def _import_gui_main():
    ...
    if hasattr(sys, '_MEIPASS'):
        meipass_gui = os.path.join(sys._MEIPASS, 'balloon', 'gui_main.py')
        if os.path.exists(meipass_gui):
            gui_main_path = meipass_gui
            # Додаємо balloon до sys.path для правильних імпортів
            balloon_path = os.path.join(sys._MEIPASS, 'balloon')
            if balloon_path not in sys.path:
                sys.path.insert(0, balloon_path)
    ...
    if os.path.exists(gui_main_path):
        ...
        spec = importlib.util.spec_from_file_location("balloon.gui_main", gui_main_path)
        ...
        gui_module.__package__ = 'balloon'
        ...
        try:
            if parent_dir not in sys.path:
                sys.path.insert(0, parent_dir)
            return importlib.import_module('balloon.gui_main')
        except ImportError:
